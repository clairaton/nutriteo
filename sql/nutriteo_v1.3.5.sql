...
CREATE TABLE IF NOT EXISTS `nutriteo`.`food_nutriment` (
  ...
  `nutriment_quantity` INT NOT NULL,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `nutriteo`.`activity` (
  ...
  `nutriment_family_id` INT NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_activity_nutriment_family1_idx` (`nutriment_family_id` ASC),
  CONSTRAINT `fk_activity_nutriment_family1`
    FOREIGN KEY (`nutriment_family_id`)
    REFERENCES `nutriteo`.`nutriment_family` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE TABLE IF NOT EXISTS `nutriteo`.`user_activity` (
  ...
  INDEX `fk_user_activity_intensity1_idx` (`intensity_id` ASC),
  CONSTRAINT `fk_user_activity_user1`
    FOREIGN KEY (`user_id`)
    REFERENCES `nutriteo`.`user` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_user_activity_intensity1`
    FOREIGN KEY (`intensity_id`)
    REFERENCES `nutriteo`.`intensity` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `nutriteo`.`user_objectives`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `nutriteo`.`user_objectives` ;

CREATE TABLE IF NOT EXISTS `nutriteo`.`user_objectives` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `user_id` INT NOT NULL,
  `nutriment_id` INT NOT NULL,
  `quantity` INT NOT NULL,
  `date_objectives` DATETIME NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_user_objectives_user1_idx` (`user_id` ASC),
  INDEX `fk_user_objectives_nutriment1_idx` (`nutriment_id` ASC),
  CONSTRAINT `fk_user_objectives_user1`
    FOREIGN KEY (`user_id`)
    REFERENCES `nutriteo`.`user` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_user_objectives_nutriment1`
    FOREIGN KEY (`nutriment_id`)
    REFERENCES `nutriteo`.`nutriment` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
